"""Learned Gibbs sampler over TabPFN-PCA64 conditional heads.

This experiment trains one scalar conditional density estimator per parameter
dimension,

    q_d(theta_d | theta_-d, e(x)),

where e(x) is the frozen TabPFN embedding reduced to PCA64. Sampling starts
from an existing saved posterior approximation and repeatedly replaces one
coordinate at a time using the learned conditionals. The resulting samples are
written in the standard `flow_vs_quantile` schema so the existing C2ST
decomposition pipeline can evaluate them unchanged.

Usage:
  uv run python scripts/learned_gibbs.py --task two_moons --seed 42
  uv run python scripts/run_c2st_sweep.py --include-method learned_gibbs --force
"""
...
@dataclass
class GibbsTrainConfig:
    """Subset of fields read by `train_flow`."""
...
def source_npz_path(
    ...
    raise FileNotFoundError(f"No initialization npz found. Tried:\n  {tried}")
...
def theta_context(
    ...
    """Build [embedding, standardized theta_-dim] contexts."""
    theta_z = (theta - theta_mean) / theta_std
    other = np.delete(theta_z, dim, axis=1)
    return np.concatenate([embeddings, other], axis=1).astype(np.float32, copy=False)


def train_conditional_heads(
    ...
    """Train D scalar conditional flows q(theta_d | theta_-d, e(x))."""
    ...
    for dim in range(dim_theta):
        ctx_train = theta_context(
            embeddings_train, theta_train, dim, theta_mean, theta_std,
        )
        ctx_val = theta_context(embeddings_val, theta_val, dim, theta_mean, theta_std)
        print(
            f"\nTraining conditional head d={dim} "
            f"(target theta_{dim}, context={ctx_train.shape[1]})..."
        )
        flow = build_flow(
            dim_theta=1,
...
def sample_scalar_conditional_batch(
    ...
    """Draw one scalar sample for each context row."""
    flow.eval()
    ...
    with torch.no_grad():
        for start in range(0, len(context), batch_size):
            ...
            samples = flow(ctx).sample()
            samples_np = samples.squeeze(-1).detach().cpu().numpy()
            out.append(samples_np)
    values = np.concatenate(out).astype(np.float32, copy=False)
    return values * np.float32(theta_std) + np.float32(theta_mean)


def gibbs_sample_observation(
    init: np.ndarray,
    ...
    """Run pseudo-Gibbs chains initialized from existing posterior samples."""
    if n_sweeps < 1:
    ...
    dim_theta = state.shape[1]
    emb = np.repeat(embedding_obs.reshape(1, -1), state.shape[0], axis=0)
    kept: list[np.ndarray] = []
    mean_abs_updates = np.zeros((n_sweeps, dim_theta), dtype=np.float32)

    for sweep in range(n_sweeps):
        if update_order == "random":
            order = rng.permutation(dim_theta)
        elif update_order == "fixed":
            order = np.arange(dim_theta)
        else:
            raise ValueError(f"Unknown update_order: {update_order}")

        for dim in order:
            old = state[:, dim].copy()
            ctx = theta_context(emb, state, int(dim), theta_mean, theta_std)
            hist = histories[int(dim)]
            state[:, dim] = sample_scalar_conditional_batch(
                flows[int(dim)],
                ctx,
                float(hist["theta_mean"][0]),
                float(hist["theta_std"][0]),
                batch_size,
            )
            mean_abs_updates[sweep, int(dim)] = float(
                np.mean(np.abs(state[:, dim] - old)),
            )
...
def choose_output_samples(
    ...
    if len(samples) > n_samples:
        ...
        idx = rng.choice(len(samples), size=n_samples, replace=True)
...
def main() -> None:
    ...
    parser.add_argument("--input-method", default="raw_tabq_ps1000",
                        help="Source posterior samples used to initialize chains.")
    ...
    output_method = args.output_method or (
        ...
        else f"{source_method}_learned_gibbs"
    )
    ...
    print(
        f"  Gibbs: sweeps={args.n_sweeps}, burn_in={args.burn_in}, "
        f"thin={args.thin}, order={args.update_order}"
    )

    print("\nSimulating train/val data...")
    data = simulate(args.task, args.n_train, args.n_val, args.seed)
    dim_theta = int(data["dim_theta"])
    defaults = get_flow_defaults(1)
    ...
    flows, histories, theta_mean, theta_std = train_conditional_heads(
        emb_train,
        emb_val,
    ...
    update_summaries = np.zeros((n_ref, args.n_sweeps, dim_theta), dtype=np.float32)
    kept_counts = np.zeros(n_ref, dtype=np.int32)
    print("\nRunning learned Gibbs sampler...")
    for obs_idx in range(n_ref):
        obs_rng = np.random.default_rng(args.seed * 1009 + obs_idx)
        samples_all, updates = gibbs_sample_observation(
        ...
        print(
            ...
            f"mean_abs_final_update={updates[-5:].mean():.4f}"
        )
    ...
    save_kwargs.update(
        ...
        learned_gibbs_embed_dim=args.embed_dim,
        learned_gibbs_n_sweeps=args.n_sweeps,
        learned_gibbs_burn_in=args.burn_in,
        learned_gibbs_thin=args.thin,
        learned_gibbs_kept_counts=kept_counts,
        learned_gibbs_update_mean_abs=update_summaries,
